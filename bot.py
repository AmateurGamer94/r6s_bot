import discord, codecs, datetime
import webbrowser

dataurl = 'https://namu.wiki/w/%s'
token = 'NjE5Mzc4NTk0NTE0NDY4ODY0.XYWC-A.LuL1OBbsBpledNT1I5KkBlRIMAg'
...
for i in range(len(f)):
    webbrowser.open(dataurl%f[i][1][1])
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('!') and message.content[1:] in commands:
            ...
            ms1 = discord.Embed(title='%s %s' % (code[0], code[1]), description='%s\n\n이름 : %s\n생일 : %s\n나이 : %s\n출생 : %s\n\n소속: %s팀\n방어 %s\n속도 %s' % (code[3], code[2], code[4], code[5], code[6], code[7], code[8][0], code[8][1]), color = color)
            ms1.set_thumbnail(url = url % ('profiles', code[0].lower()))
            ms1.set_image(url = url % ('portrait', code[0].lower()))
            ms2 = discord.Embed(title='특수 장비', description='' % (code[2], code[3], code[4], code[5], code[6], code[7], code[8][0], code[8][1]), color = color)
            ms2.set_thumbnail(url = url % ('profiles', code[0].lower()))
            ms2.set_image(url = url % ('portrait', code[0].lower()))
            now=datetime.datetime.now()
            ms2.set_footer(text = '%s년 %s월 %s일 %s %s:%s' % (now.year, now.month, now.day, '오전' if now.hour < 13 else '오후', now.hour if now.hour < 13 else now.hour - 12, now.minute))
